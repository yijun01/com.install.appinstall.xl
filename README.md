...
3. **检测退出拦截**：拦截应用因检测到伪造包触发的退出行为，支持**普通拦截/超强拦截**双模式
4. **权限防护**：可自动授权QUERY_ALL_PACKAGES等检测相关核心权限，其余权限可自行选择是否需要虚假授权
